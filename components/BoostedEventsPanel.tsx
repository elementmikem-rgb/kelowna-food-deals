...
import type { BoostedEvent, EventOption, VenueOption } from "@/lib/sponsored-data";
import { formatCheckedAt } from "@/lib/time";

const DURATIONS = [3, 7, 14] as const;

export function BoostedEventsPanel({
  active,
  venueOptions,
  eventOptions,
}: {
  ...
  const eventsForVenue = useMemo(
    () => eventOptions.filter((e) => e.venueId === venueId),
    [eventOptions, venueId]
  );
  ...
  async function post(id: number, value: number | null) {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/admin/sponsored/events/${id}`, {
        method: "POST",
  ...
  return (
    <section className="flex flex-col gap-3">
      <h2 className="font-display text-xl text-foreground">Boosted events</h2>
      <p className="text-sm text-muted">
        A boosted event sorts first within its venue&rsquo;s card on the events board and gets a
        &ldquo;Featured&rdquo; badge until its window ends.
      </p>
          ...
          {active.map((e) => (
            ...
            >
              <div className="flex flex-col">
                ...
                <span className="text-xs text-muted-2">until {formatCheckedAt(e.boostedUntil)}</span>
              </div>
              <button
                onClick={() => post(e.id, null)}
                disabled={busy}
      ...
      <div className="flex flex-wrap items-center gap-2 rounded-lg border border-border bg-surface-raised p-3">
        ...
        >
          <option value="">
            {venueId ? "Select an event…" : "Pick a venue first"}
          </option>
          {eventsForVenue.map((e) => (
        ...
        <button
          onClick={() => eventId && post(eventId, days)}
